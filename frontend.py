from tkinter import *
import backend # (insert, view, search, delete).

def get_selected_row(event):
    global selected_row
    index = list.curselection()[0]   # Get the index of the selected row in the Listbox
    selected_row = list.get(index)   # Get the full data tuple for that row

    # Fill the Entry boxes with the selected row's data
    e1.delete(0,END) # clears whatever is currently in the Date entry box.
    e1.insert(END,selected_row)

    e2.delete(0,END)
    e2.insert(END,selected_row)

    e3.delete(0,END)
    e3.insert(END,selected_row)

    e4.delete(0,END)
    e4.insert(END,selected_row)

    e5.delete(0,END)
    e5.insert(END,selected_row)

    e6.delete(0,END)
    e6.insert(END,selected_row)
...
def view_command():
    list.delete(0,END) # clears the Listbox.
